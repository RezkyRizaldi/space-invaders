...
const createParticles = ({ object, color = '#BAA0DE', fades }) => {
	for (let i = 0; i < 15; i++) {
		particles.push(
			new Particle({
				position: {
...
const animate = () => {
	...
	invaderProjectiles.forEach((projectile, index) => {
		...
		if (projectile.position.y + projectile.height >= player.position.y && projectile.position.x + projectile.width >= player.position.x && projectile.position.x <= player.position.x + player.width) {
